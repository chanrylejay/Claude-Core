...
const leanParkedPaths = live.coldStart.filter((e) => /lean:lookup/.test(e.comment)).map((e) => e.path);
const leanParkedChars = leanParkedPaths.reduce((sum, p) => sum + charsOf(ROOT, p), 0);
t(`R2, the scenario itself (behavior): LEAN's resolver plan is three files and the live lean:lookup set plus the canon are lookups; a by-hand list that ignores the carve-out would add ${leanParkedChars} chars the resolver parks`, (() => {
  const r = resolveBoot(live, { seat: "codex", mode: "LEAN" });
  const parked = r.lookup.filter((e) => /LEAN skips/.test(e.why)).map((e) => e.path);
  const extra = parked.filter((p) => p !== live.state.active_project).reduce((sum, p) => sum + charsOf(ROOT, p), 0);
  return r.boot.length === 3 && eq([...parked].sort(), [...leanParkedPaths, live.state.active_project].sort())
    && leanParkedPaths.every((p) => !paths(r.boot).includes(p)) && extra > 0;
})());
// Batch 2b: canon measured 7611 chars at this cut; its size is reported, never pinned.
const recoveryCanonChars = charsOf(ROOT, live.state.active_project);
t(`R2 v2, the scenario itself (behavior): under an UNKNOWN workspace mode the live resolver keeps the canon in BOOT (${recoveryCanonChars} chars) and drops only the mode set`, (() => { const r = resolveBoot(live, { seat: "codex", mode: "NOT_A_MODE" }); const c = live.state.active_project; return paths(r.boot).includes(c) && !paths(r.lookup).includes(c) && charsOf(ROOT, c) > 0 && /not in the modes block/.test(r.problems[0]) && eq(paths(r.boot), ["CLAUDE.md", "memory/MEMORY.md", ...live.coldStart.map((e) => e.path), c]); })());
