...
t("LIVE: the four routed reads (cost file, codex.md, DIRECTORY, REGISTRY) are in no seat's BOOT list in any mode, and each is verified present (a LOOKUP line or an index line)", Object.keys(SEATS).every((seat) => liveModes.every((m) => !paths(resolveBoot(live, { seat, mode: m }).boot).some((p) => ROUTED.includes(p)))) && ROUTED.every((p) => fs.existsSync(path.join(ROOT, p))) && ["lessons/platforms/codex.md", "DIRECTORY.md", "projects/REGISTRY.md"].every((p) => live.lookup.some((l) => l.path === p)) && /chan-ai-cost-context\.md\) — open before DeepSeek CLI work/.test(fs.readFileSync(path.join(ROOT, "memory/MEMORY.md"), "utf8")));
...
// other mode", unknown included, where the resolver keeps it (6,746 chars apart on the recovery path).
...
t("R2 v2, the scenario itself (behavior): under an UNKNOWN workspace mode the live resolver keeps the canon in BOOT (6,746 chars a by-hand list that parks it would drop from the recovery path) and drops only the mode set", (() => { const r = resolveBoot(live, { seat: "codex", mode: "NOT_A_MODE" }); const c = live.state.active_project; return paths(r.boot).includes(c) && !paths(r.lookup).includes(c) && charsOf(ROOT, c) === 6746 && /not in the modes block/.test(r.problems[0]) && eq(paths(r.boot), ["CLAUDE.md", "memory/MEMORY.md", ...live.coldStart.map((e) => e.path), c]); })());
