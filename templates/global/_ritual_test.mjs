...
rr = nspawn(NA);
t("nudge: yellow crossing speaks once WITH the relay mandate", /yellow band: 163K/.test(said(rr)) && /RELAY THIS LINE VERBATIM/.test(said(rr)) && /your voice is the display/.test(said(rr)));
rr = nspawn(NA);
...
rr = nspawn(NA);
t("nudge: red crossing speaks with the free-move advice AND the mandate", /RED band: 382K/.test(said(rr)) && /\/clear is the free move/.test(said(rr)) && /RELAY THIS LINE VERBATIM/.test(said(rr)));
...
fs.writeFileSync(NA, JSON.stringify({ message: { usage: { input_tokens: 2000, cache_read_input_tokens: 380000 } } }) + "\n");
rr = nspawn(NA);
t("nudge: a RE-crossing after the step-down speaks again", /RED band: 382K/.test(said(rr)));
