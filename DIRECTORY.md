...
- `codex/` — Codex guard layers (Sep 1 2026): `codex-guard.mjs` imports the existing push parser, consumes only Chan's strict repo-bound `~/.codex/PUSH_GO`, and blocks remote rewrites; `hooks.json` is the user-level wiring skeleton with structured JSON denies; `_codexguard_test.mjs` stages the launcher without touching the live home. `pre-push` is the byte-identical, per-clone Git gate and `_prepush_test.mjs` proves its strict, one-shot token behavior; the arming check must confirm `<clone>/.git/hooks/pre-push` is installed and equal. Live-fire stays mandatory.
