...
- `hooks/` — push-guard.mjs (PreToolUse: `git push` blocked unless a one-shot GO file exists) + gauntlet-guard.mjs (Stop: a build turn can't claim "done" without gauntlet tokens) + `_pushguard_test.mjs` (28 cases) / `_gauntlet_test.mjs` (33 cases, six of them sandboxed behaviour checks that spawn the real hook) — in-kit regression nets, plain node, MANDATORY re-run after any hook edit. The session-ritual hook has one at `templates/global/_ritual_test.mjs` (9 cases, tests the INSTALLED hook, not the template copy). The switch script has its own at `templates/_switch_test.mjs` (13 cases; always passes --no-env, because that script writes MACHINE-WIDE env vars that no sandbox contains).
