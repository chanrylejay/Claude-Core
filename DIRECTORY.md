...
- `boot-claudeai.mjs` — the browser half's mechanical boot (run from repo root right after the clone): prints CLONE HEAD freshness, resolves the state block + cold_start + mode set from the memory index frontmatter, prints the ordered RAW read list, exits 1 if any listed file is missing. Replaces prose-parsing, never the reading. + `_boot_claudeai_test.mjs`, its net (8 pins: intact boot, freshness signal, default + override mode resolution, dedup, and one mutation per failure mode — unknown mode, deleted mode file, stripped cold_start).
