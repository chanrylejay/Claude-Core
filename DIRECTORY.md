...
- `switch-to-codex.md` — the Codex cutover runbook: second-hands posture, project trust and account condition, Codex gate differences, and rollback to the DeepSeek CLI.
...
- `_policy_diff.mjs` — the L24 companion for POLICY rewrites (Sep 9 2026, batch 0b): sentence-aligns old vs new and reports every clause whose modal/negation/scope word-set changed, plus dropped and added policy sentences; a review LIST for a peer, never a verdict (`--git <ref> <path>`, `--self-test`) + `_policy_diff_test.mjs`, its net (pins the negation flip L24 cannot see, a clean relocation, and a dropped sentence).
...
- `codex/codex-guard.mjs` — 0c-fix1 Bash checks: literal --no-verify before push detection, protected core.hooksPath, plain subcommand tokens, and case-sensitive -C repository binding. Script-file/xargs/fresh-clone gaps remain; lessons/platforms/codex.md owns their limits.
...
- `codex/fixtures/synthetic-bash-cases.json` — 0c-fix1 synthetic Bash refusals and permitted forms, replayed through configured wiring with temporary-home tokens; no command is executed.
...
- `hooks/` — push-guard.mjs (PreToolUse: `git push` blocked unless a one-shot GO file exists) + gauntlet-guard.mjs (Stop: a build turn can't claim "done" without gauntlet tokens) + `_pushguard_test.mjs` / `_gauntlet_test.mjs` (some of them sandboxed behaviour checks that spawn the real hook) — in-kit regression nets, plain node, MANDATORY re-run after any hook edit. `playwright-guard.mjs` (Sep 4 2026, cli-playwright-1; PreToolUse: denies playwright-cli / `npx @playwright/cli` navigation beyond `localhost`/`127.0.0.1` and blocks its `upload`/`drop`; wired in ~/.claude/settings.json beside push-guard, same `matcher` scope) + `_playwrightguard_test.mjs` (23 pins) + `fixtures/pretooluse-bash-playwright-{external-open,upload}.json` (net-pinned captured payloads). The session-ritual hook has one at `templates/global/_ritual_test.mjs` (tests the INSTALLED hook, not the template copy). The switch script has its own at `templates/_switch_test.mjs`. Assertion counts are deliberately NOT listed here: each net prints its own count when it runs, and a number written here goes stale silently (it did — this line claimed 28/33/9/13 against a real 38/47/22/19 on Jul 25 2026). Run them for the current numbers, and read each script's own header for how it must be invoked; no invocation rule is duplicated into this file.
- `cli/` — the DeepSeek (Claude Boss) CLI hand's operational homes (Sep 4 2026, cli-playwright-1): `cli/playwright-cli/SKILL.md` is the one-home source of the DeepSeek `playwright-cli` skill, installed byte-equal to `~/.claude/skills/playwright-cli/SKILL.md` — disk-first workflow (snapshot/screenshot to file), localhost-only navigation, every screenshot reported AWAITING HIS LOOK (blind-hands rule), never claimed as visual verification. The tool-playbook is the state pointer; the guard is `hooks/playwright-guard.mjs`.
