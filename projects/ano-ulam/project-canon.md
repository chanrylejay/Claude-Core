...
(These are ano-ulam's PROJECT-APPLIED instances with the real column names. The universal form of each rule, e.g. Neon NUMERIC-returns-STRINGS, ON CONFLICT DO NOTHING, ORDER BY DESC NULLS LAST, is single-homed in ../../lessons/platforms/neon-postgres.md.)
...
(The universal Vercel-cron rule, GET-delegates-POST + dual-auth + no-store headers, is single-homed in ../../lessons/platforms/vercel.md; below is ano-ulam's application plus its project-only facts.)
