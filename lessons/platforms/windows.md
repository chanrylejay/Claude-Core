...
- The owner's terminal is PowerShell, not Git Bash. `/c/...` paths become `C:\c...` and fail; PowerShell needs native `C:\Users\...` paths. Ask which shell before handing over a command line. Separately, the kit's push guard blocks PowerShell chains by design, so a push routed through PowerShell is correctly denied and is not the same defect as the guard's repository mismatch.
