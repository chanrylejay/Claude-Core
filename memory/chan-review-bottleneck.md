...
6. **When he is unreachable, PARK — the law is [[chan-hard-rules]] rule 8 and it governs here**
   (work only what needs no decision from him, queue every decision for his return, deciding one
   yourself while he is away is forbidden). Applied to THIS file's job: one line per parked item
   in the tracked backlog — the question, the options, where it blocked — surfaced where it
   blocked, never batched into a pile at day's end, and silence is never approval.
