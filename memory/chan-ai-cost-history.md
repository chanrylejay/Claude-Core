---
name: chan-ai-cost-history
description: The history behind the AI cost posture, moved verbatim out of chan-ai-cost-context on Sep 14 2026 (batch 2a) so the law file stays short. The arc from company-paid Claude to Chan's own DeepSeek key, the first-contact proof, the measured session costs, the Aug 2026 audit re-ranking, the gauge verification, the cache-mechanics source, the market note, and the free-tier toolbelt. History, never law: the rules live in chan-ai-cost-context.
metadata:
  type: reference
---

# AI cost history (the why behind [[chan-ai-cost-context]]; moved Sep 14 2026, batch 2a)

The law and the current posture live in [[chan-ai-cost-context]], the file the contract makes LAW before DeepSeek CLI work. Nothing here binds a session; it is the record of how the posture came to be, kept as written (dates inside).
...
**Codex posture as first written (Sep 1-2 2026; PRIMARY hand since Sep 6 2026, Chan's word):** Codex is the primary hand and the DeepSeek CLI is the secondary, never a replacement, until Chan says otherwise (Chan, Sep 9 2026). Chan is on a free ChatGPT Plus month as of Sep 9 2026. Claude-Core upgrade work runs gpt-6-astra (since Sep 8 2026; a peer to the browser seat); normal project work such as ESS stayed on gpt-5.6-terra at medium effort, the recommended setting, until Chan's Sep 14 2026 word moved project sessions to luna at xhigh (the posture line in the law file). Effort is a session-open setting, not a per-turn one. Its usage is not charged to the DeepSeek key, so the DeepSeek meter and peak windows do not decide its work; the kit-lean law still does. It runs only under Chan's own account: any client-account, client-funded account, or cutover changes authority and needs Chan's explicit ruling before use. The free month ends on its own; Chan says when it ends and when the primacy changes, nobody infers either, so revisit the posture on his word instead of silently treating it as permanent.

**The switch, as it happened (posture last updated Jul 29 2026, the switch DONE):**
...
- The first practical note (Jul 29, SUPERSEDED Aug 24 2026): the window is not smaller — v4-flash carries a 1M-token context — but a big window is a cost trap under the Aug 16 pricing, so the compact-early habit replaces the old worry. The cost of that first session was about $2 (374k tokens), Chan paying his own key for the first time. /compact and /info still work the same way.
...
- The 10/10 workflow goal was always a system that SURVIVES this downgrade (the kit-lean law, still in the law file). That design is now live reality, not insurance.
- The kit-lean law's second sentence was reworded Aug 24 2026 on Chan's fairness ruling — the CLI is a collaborator, not a lesser; the same session it was reworded, the CLI caught a defect the planner's clean sandbox could not see.
...
reconstruction: ../lessons/platforms/deepseek-api.md — one home (the rule that it is read
before any cost reasoning stayed in the law file). What changed in one line: off-peak is ~2x the old flat rate, PEAK (09:00-12:00
...
0. **MASTER HABIT — keep the context small.** And now you can SEE it (Aug 2026, Chan's
   ...
   us: when it goes red at a task boundary, compact or /clear. In the VS Code extension panel (no statusline surface) the SAME thresholds speak as one-time [gauge] lines via the session hook on band crossings — silent otherwise, parity-pinned to the meter, and the line MANDATES verbatim relay because the panel renders hook output only through the model voice (Chan eyes Aug 25; VERIFIED end-to-end Aug 28 2026 — a live
[gauge] yellow crossing on Chan's extension panel, screenshot reviewed by claude.ai against
the hook's five checks). Everything below shrinks with it: at 100K a cold
...
The four session habits, in order of dollars saved, with the measurements behind them:
...
Related: [[chan-ai-cost-context]] · [[chan-hard-rules]] · [[chan-career-portfolio-state]]
