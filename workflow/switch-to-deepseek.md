...
- Main model: `deepseek-v4-flash` (Chan chose flash Jul 24 2026 for cost/speed; for the smarter, pricier tier flip ALL FIVE model keys to `deepseek-v4-pro`: ANTHROPIC_MODEL, the three ANTHROPIC_DEFAULT_* keys, and CLAUDE_CODE_SUBAGENT_MODEL. Flipping only the first three leaves every subagent on flash after what was meant to be an upgrade, with nothing to surface it). Opus/Sonnet names route to
