...
off with dates as they happen.

## 1. Status flips across the kit — DONE Jul 24 2026 (by Claude, closure day)

- chan-resigned-devoted-jul15.md: status ENDED, handoff complete, fully hands-off.
- memory/MEMORY.md: NOW block + index lines flipped to ENDED.
...
(00-SEALED.md in the old project memory) and the ORIGIN PROJECT'S archives stay untouched, meaning: never READ from them, never quote them, never work from them, and never act on what is in them. That seal covers the old project's material only. Claude-Core's own archives/ folder is different: it is in-kit provenance — the frozen source docs this kit descends from — and stays readable for source checks, with the standing line that its originals hold client names and terms, so nothing client-identifying is ever reproduced out of them. (Audit Jul 26 2026: "the archives" stood unqualified here while DIRECTORY.md treats archives/ as a readable frozen reference and the project checklist's own header cites it as source — two files disagreed on whether the kit may read its own provenance. The scoping is Claude's call from in-kit evidence; Chan, say the word if the seal was meant to cover the kit's own archives too and this reverts.) COPYING them is a different act and is allowed — a backup, a migration, or a move to cold storage preserves the seal, and each of those still happens only on Chan's word.
...
## 4. Resume + portfolio site sweep (GO-gated push)

- [ ] resume-v4.html: end date + past tense for Devoted. Then regenerate resume.pdf FROM it
  (open, Ctrl+P, save over resume.pdf; never edit the PDF directly).
- [ ] Portfolio site: any "current/ongoing" wording for Devoted flips to completed. Push needs
  GO (live deploy).
...
- [ ] n8n pre-upgrade backup (~500MB, .n8n-backup-2026-07-23): delete once he is confident.
- [ ] Sealed Devoted memory bank long-term storage: keep in place (current ruling) or zip to
  cold storage. Decide here, no rush.
...
- [ ] Interview prep: rehearse the Devoted narrative (problem, method, shipped outcomes,
  lessons) per the career playbook; the living demo IS the centerpiece.
