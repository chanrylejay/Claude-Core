...
- **SUPERSEDED FOR CODEX (Chan, Sep 3 2026):** the Sep 2 Lean routing experiment is retired from
  Codex after the shared Lean surfaces coincided with the DeepSeek ritual-net failure. Codex has no
  Lean MCP or skill: establish a skeleton and outline first, then use `rg` plus bounded line ranges
  rather than whole-file reads; boot, drill, memory, L24, exact-value verification, and any exact
  wording stay raw. This is a Codex-only routing rule, not a change to the DeepSeek CLI's Lean
  setup.
  Historical measurement retained, not active routing: on `ess-system` main,
  `components/sales-client.tsx` (largest non-lock source) was 40,684 raw characters versus 3,559
  Lean overview (-91.3%); two reads were 81,368 versus 7,118 (-91.3%), and a `getTaskUrgency`
  search was 1,410 versus 1,251 (-11.3%). This supersedes the tiny-canary hold-off in PASTE-45.
