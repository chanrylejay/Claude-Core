...
Chan runs TWO agents and is the bridge between them. Neither is better: different reach, equal
standing — one lives in a disposable sandbox, the other on the real machine.
- **claude.ai (Opus 5, browser)** — plans, writes the briefs, does hostile review, and is the
  ONLY one that can see images. Since Aug 2026 it also RUNS CODE in a per-conversation sandbox:
  it clones the public repo, builds and TESTS patches against this kit's own nets, and hands
  Chan finished files. Its disk is disposable and it holds no credentials, so it cannot push,
  cannot touch the live machine, and cannot reach a private repo — the repo stays PUBLIC for
  exactly that reason (README).
- **Claude Boss CLI (DeepSeek v4 flash, VS Code)** — the only hands on the real machine: reads
  the repo, writes code, runs tests, installs hooks, commits, pushes. CANNOT see images: it
  saves screenshots to a file and hands Chan the path.
- **Chan** — owns every decision, couriers files and images between the two, and gives the GO
  every time.
claude.ai builds and proves, the CLI lands and pushes, Chan approves. Mechanics:
`workflow/two-model-relay.md`. (Superseded Aug 24 2026: this section claimed claude.ai "cannot
run code, write to disk, or push" — two shipped, net-proven batches falsified that.)

## Rule 0 — THE DRILL
After any compaction or fresh start: don't trust the summary; OPEN the READ-FIRST files
themselves; verify actual disk/repo state before the first substantive reply — disk wins.
The hub's SessionStart ritual supplies the reading list; no hub means treat a fresh start as a
compaction. The AUTHORITATIVE text and full mechanics live in ONE home:
`workflow/the-drill-and-memory.md` (de-duplicated Aug 24 2026; this used to be the third copy).
...
- **Ship in batches, GO-gated.** Related fixes ship as ONE push, and NO deploy-costing action
  happens without his explicit GO, each time — approval once is never approval forever. What
  counts as deploy-costing, the example list, and the if-unsure-it-is boundary live in ONE home:
  hard rules 6-7 (`memory/chan-hard-rules.md`) (de-duplicated Aug 24 2026).
...
- **Verify visually.** A UI change is VERIFIED only when Chan's eyes have seen a screenshot of
  ...
  (`memory/chan-hard-rules.md`) (de-duplicated Aug 24 2026).
